...
def make_temp_config_for_method(base_config, csv_row, method):
    ...
    if method == "ifl-gr":
        ...
        if "learning_rate" in csv_row and csv_row["learning_rate"] != "":
            cora_updates["learning_rate"] = float(csv_row["learning_rate"])
...
def method_pipeline_2stage(grace_dir, base_config, method, grid_script_2stage, grid_csv_name, args, baseline_robust, out_csv_path):
    """
    Pipeline for two-stage search methods (IFL-GR, IFL-GC).
    Calls 2-stage grid search script and then verifies top candidates.
    """
    print(f"\n=== [{method}] two-stage grid search ===")
    run_grid_script(
        grace_dir=grace_dir,
        script_name=grid_script_2stage,
        gpu_id=args.gpu_id,
    ...
    if not top_rows:
        print(f"[{method}] No candidates found in {grid_csv_path}.")
        return

    print(f"=== [{method}] top-{len(top_rows)} from two-stage (representative runs) ===")
...
def main():
    parser = argparse.ArgumentParser(
        description="Two-stage automate: GRACE baseline + IFL-GR/IFL-GC two-stage search + GCA one-stage search"
    )
    ...
    parser.add_argument(
        "--out",
        type=str,
        default=os.path.join("results", "cora_full_pipeline_2stage_results.csv"),
    ...
    # Two-stage methods
    method_pipeline_2stage(
        grace_dir=grace_dir,
        base_config=base_config,
        method="ifl-gr",
        grid_script_2stage="grid_search_iflgr_cora_2stage.py",
        grid_csv_name="grid_search_iflgr_cora_2stage_results.csv",
        args=args,
    ...
    method_pipeline_2stage(
        ...
        base_config=base_config,
        method="ifl-gc",
        grid_script_2stage="grid_search_iflgc_cora_2stage.py",
        grid_csv_name="grid_search_iflgc_cora_2stage_results.csv",
        args=args,
        baseline_robust=baseline_robust,
        out_csv_path=out_path,
    )

    # GCA one-stage (for comparison)
    # For now, skip GCA in two-stage pipeline; user can run separately
    print(
        "\n[Note] GCA uses one-stage search. Run separately: "
        "python GRACE/tools/grid_search_gca_cora.py"
    )
