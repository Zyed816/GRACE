...
def main():
    parser = argparse.ArgumentParser(description="Grid search for GCA on Cora")
    parser.add_argument("--config", type=str, default="config.yaml")
    parser.add_argument("--gpu_id", type=int, default=0)
    parser.add_argument("--std_weight", type=float, default=0.5)
    parser.add_argument("--topk", type=int, default=10)
    parser.add_argument("--out", type=str, default="results/grid_search_gca_cora_results.csv")
    ...
    for values in values_product:
        for fp in feature_profiles:
            ...
            try:
                metrics, _ = run_train(grace_dir, temp_cfg, method="gca", gpu_id=args.gpu_id)
