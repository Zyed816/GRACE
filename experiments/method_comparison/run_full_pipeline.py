...
def run_train(grace_dir, config_path, dataset, method, gpu_id, verbose_output=False):
    start = t()
...
def run_grid_script(grace_dir, script_name, gpu_id, topk, std_weight, dataset):
    ...
    cmd = [
        sys.executable,
        os.path.join("experiments", "method_comparison", script_name),
        "--gpu_id",
        str(gpu_id),
        "--topk",
        str(topk),
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Stream raw train.py logs instead of only start/done summary lines.",
    )
    ...
    method_pipeline(
        ...
        dataset_key=args.dataset,
        method="gca",
        grid_script="grid_search_gca.py",
